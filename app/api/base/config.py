...
@config.get("/one", summary="获取指定配置", dependencies=[Depends(JwtUtil.check_login)])
...
async def one(
    ...
    return RES.res_200(
        Config_Pydantic.model_validate(res).model_dump(mode="json", include=fields)
    )


@config.get("/all", summary="获取全部配置", dependencies=[Depends(JwtUtil.check_login)])
@handle_api_exceptions
async def one(
    ...
    res = await Config_Pydantic.from_queryset(
        ConfigModel.all().offset(limit * (page - 1)).limit(limit)
    )
...
@config.get("/column", summary="查询列", dependencies=[Depends(JwtUtil.check_login)])
@handle_api_exceptions
async def column(
    fields: Optional[List[str]] = Query(default=None, description="查询字段")
):
...
@config.put("/update", summary="更新配置", dependencies=[Depends(JwtUtil.check_login)])
@handle_api_exceptions
async def update(data: ConfigVal):
    res = await ConfigModel.filter(name=data.name).update(
        value=data.value, update_time=int(timezone.now().timestamp() * 1000)
    )
