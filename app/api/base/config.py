...
@config.get("/one", summary="获取指定配置", dependencies=[Depends(JwtUtil.check_admin)])
...
@config.get("/all", summary="获取全部配置", dependencies=[Depends(JwtUtil.check_admin)])
@handle_api_exceptions
async def one(
...
@config.get("/column", summary="查询列", dependencies=[Depends(JwtUtil.check_admin)])
@handle_api_exceptions
async def column(
    fields: Optional[List[str]] = Query(default=None, description="查询字段")
):
...
@config.put("/update", summary="更新配置", dependencies=[Depends(JwtUtil.check_admin)])
@handle_api_exceptions
